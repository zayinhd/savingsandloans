...
Public Class AdminAdmin
    ...
    Private Sub LoadAdmins()
        Try
            Using conn As New MySqlConnection(connectionString)
                ...
                Dim query As String = "SELECT id, FirstName, LastName, Email, role FROM users WHERE role = 'admin'"
                Dim adapter As New MySqlDataAdapter(query, conn)
                Dim dt As New DataTable()
                adapter.Fill(dt)

                AdminGrid.DataSource = dt ' Make sure you have a DataGridView named AdminsGrid on your form

                ' Optional: styling for teal text on white background
                For Each row As DataGridViewRow In AdminGrid.Rows
    ...
    Private Sub EditBtn_Click(sender As Object, e As EventArgs) Handles EditBtn.Click
        Try
            Using conn As New MySqlConnection(connectionString)
                conn.Open()
                Dim query As String = "UPDATE users SET FirstName=@FirstName, LastName=@LastName, Email=@Email,
                                   Username=@Username, Password=@Password, Role=@Role, Date_of_birth=@DOB WHERE id=@id"
                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@FirstName", FirstName.Text.Trim())
                    cmd.Parameters.AddWithValue("@LastName", LastName.Text.Trim())
            ...
            If DateTime.TryParse(DateOfBirth.Text.Trim(), dob) Then
                ...
                Using conn As New MySqlConnection(connectionString)
                    ...
                    Using cmd As New MySqlCommand(query, conn)
                        ...
                        LoadAdmins() ' Reload the customers list after addition
                    End Using
                End Using
            Else
